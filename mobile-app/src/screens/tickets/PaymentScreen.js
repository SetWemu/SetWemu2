...
const PaymentScreen = ({ route, navigation }) => {
  ...
  const Method = ({ id, label, icon: Icon, desc }) => (
    <TouchableOpacity
      style={[ps.card, method === id && ps.cardActive]}
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={ps.desc}>{desc}</Text>
      </View>
      <View style={[ps.radio, method === id && ps.radioActive]}>
        {method === id && <Check size={10} color="#141416" weight="bold" />}
      </View>
    </TouchableOpacity>
  );
    ...
    <SafeAreaView style={ps.safe}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        <Method
          id="pickme"
          label="PickMe Pay"
          desc="Fast checkout via PickMe"
          icon={Wallet}
        />
        <Method
          id="bank"
          label="Bank Transfer"
          desc="Manual deposit / KOKO"
          icon={Bank}
        />
      </ScrollView>
      ...
      <View style={ps.footer}>
        ...
        <TouchableOpacity
          style={ps.btn}
          onPress={() => navigation.navigate('Success', { event })}
        >
