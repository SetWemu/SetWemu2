    ...
    <SafeAreaView style={s.container}>
      ...
      >
        <Section title="Order Summary">
          <View style={s.eventCard}>
            <Image
              source={{ uri: event.hero_images?.[0] }}
              style={s.eventImage}
            />
            <View style={s.eventInfo}>
              ...
              <Text style={s.eventMeta}>
                {event.time} • {quantity}× {ticket.type}
              </Text>
              <Text style={s.blueLabel}>
                LKR {ticket.price.toLocaleString()} / ticket
              </Text>
            </View>
          </View>
        </Section>
        ...
        <Section title="Promo Code">
          <View style={s.promoRow}>
            ...
            <TouchableOpacity
              style={[
                s.applyBtn,
                discount > 0 && { backgroundColor: C.success },
              ]}
        ...
        <Section title="Payment Details" noBorder>
          <View style={s.priceCard}>
            <View style={s.priceRow}>
              ...
              <Text style={s.priceVal}>LKR {subtotal.toLocaleString()}</Text>
            </View>
            <View style={s.priceRow}>
              <Text style={s.priceLabel}>Service Fee (5%)</Text>
              <Text style={s.priceVal}>LKR {serviceFee.toFixed(2)}</Text>
            </View>
            {discount > 0 && (
              <View style={s.priceRow}>
                <Text style={[s.priceLabel, { color: C.success }]}>
                  Discount
                </Text>
                <Text style={[s.priceVal, { color: C.success }]}>
            ...
            <View style={s.priceRow}>
              ...
              <Text style={s.totalVal}>LKR {finalTotal.toLocaleString()}</Text>
            </View>
          </View>
        </Section>
        ...
        <View style={s.policySection}>
          {['terms', 'refund'].map(type => (
            <TouchableOpacity
              key={type}
              style={s.checkRow}
              onPress={() => setAgreed(p => ({ ...p, [type]: !p[type] }))}
            >
              <CheckCircle
                size={22}
                color={agreed[type] ? C.blue.light : C.text.tertiary}
                weight={agreed[type] ? 'fill' : 'regular'}
              />
              <Text style={s.checkText}>
                I agree to the{' '}
                <Text style={s.link}>
                  {type === 'terms' ? 'Terms' : 'Refund Policy'}
                </Text>
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <View style={s.footer}>
        <View>
          ...
          <Text style={s.footerPrice}>LKR {finalTotal.toLocaleString()}</Text>
        </View>
        <TouchableOpacity
          style={[
            s.payBtn,
            (!agreed.terms || !agreed.refund) && { opacity: 0.4 },
          ]}
