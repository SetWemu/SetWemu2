...
import { 
  ArrowLeft, MapPin, CalendarBlank, Clock, Tag, Users,
  Ticket, Question, ShareNetwork, Heart
              ...
              {tickets.map((ticket, index) => (
                <View key={index} style={s.ticketCard}>
                  ...
                  <View style={s.ticketInfo}>
                    <Text style={s.ticketType}>{ticket.type || 'Ticket Type'}</Text>
            ...
            <View style={s.section}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.gallery}>
                {galleryImages.map((img, index) => (
                  <Image key={index} source={{ uri: img }} style={s.galleryImage} />
                ))}
              </ScrollView>
            </View>
          )}
              ...
              {faqs.map((faq, index) => (
                <View key={index} style={s.faqCard}>
                  <View style={s.faqHeader}>
                    ...
                    <Text style={s.faqQuestion}>{faq.question || 'Question'}</Text>
                  </View>
...
const s = StyleSheet.create({
  ...
  heroPlaceholder: { width: '100%', height: '100%', backgroundColor: C.bg.card, alignItems: 'center', justifyContent: 'center' },
  heroPlaceholderText: { fontSize: 16, color: C.text.tertiary, fontWeight: '600' },
  heroOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 120, backgroundColor: 'rgba(0,0,0,0.4)' },

  // Header
  header: { position: 'absolute', top: 50, left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20 },
  headerBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(10px)' },
  headerActions: { flexDirection: 'row', gap: 10 },

  // Preview Badge
  previewBadge: { position: 'absolute', top: 100, right: 20, backgroundColor: C.blue.light, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
  previewBadgeText: { fontSize: 10, fontWeight: '900', color: '#141416', letterSpacing: 1 },
  ...
  tagBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: C.bg.elevated, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, borderWidth: 1, borderColor: C.border.light },
  tagText: { fontSize: 11, fontWeight: '700', color: C.blue.light },

  // Title
  title: { fontSize: 26, fontWeight: '900', color: C.text.primary, marginBottom: 16, lineHeight: 32, letterSpacing: -0.5 },
  ...
  // Sections
  section: { marginBottom: 24, paddingTop: 20, borderTopWidth: 1, borderTopColor: C.border.subtle },
  sectionTitle: { fontSize: 18, fontWeight: '800', color: C.text.primary, marginBottom: 14, letterSpacing: -0.2 },
  description: { fontSize: 15, color: C.text.secondary, lineHeight: 24 },

  // Tickets
  ticketCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: C.bg.card, borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: C.border.light },
  ticketIcon: { width: 40, height: 40, borderRadius: 12, backgroundColor: C.bg.elevated, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  ticketInfo: { flex: 1 },
  ticketType: { fontSize: 15, fontWeight: '700', color: C.text.primary, marginBottom: 4 },
  ...
  // FAQ
  faqCard: { backgroundColor: C.bg.card, borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: C.border.light },
  faqHeader: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10 },
  faqQuestion: { flex: 1, fontSize: 14, fontWeight: '700', color: C.text.primary },
  faqAnswer: { fontSize: 13, color: C.text.secondary, lineHeight: 20 },

  // Bottom Bar
  bottomBar: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: 20, paddingBottom: 35, backgroundColor: C.bg.card, borderTopWidth: 1, borderTopColor: C.border.subtle },
  backButton: { backgroundColor: C.bg.elevated, borderRadius: 14, paddingVertical: 14, alignItems: 'center', borderWidth: 1, borderColor: C.border.light },
